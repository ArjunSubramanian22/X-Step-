...
export default function DashboardScreen() {
  ...
  const { footData, isDarkMode, calculateRiskScore, thresholds, history, activeAlertCount, gaitAnalysis, apiOnline } = useSensor();
  ...
  useEffect(() => {
    refreshRecommendations(history, activeAlertCount, completionRate, gaitAnalysis.gaitPattern);
  }, [history, activeAlertCount, completionRate, gaitAnalysis.gaitPattern, refreshRecommendations]);
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background, paddingTop: Platform.OS === 'web' ? insets.top : 0 }]}>
      ...
      >
        <View style={styles.header}>
          <View>
            ...
            <Text style={[styles.subtitle, { color: colors.textSecondary }]}>
              ...
              {gaitAnalysis.gaitPattern !== 'unknown' ? ` • ${gaitAnalysis.gaitPattern.replace(/_/g, ' ')}` : ''}
              {apiOnline ? ' • live ML' : ' • on-device ML'}
            </Text>
          </View>
        </View>
        ...
        <View style={[styles.healthIndexCard, { backgroundColor: colors.surface, marginTop: 0 }]}>
          <Text style={[styles.healthIndexTitle, { color: colors.text, marginBottom: 8 }]}>Live gait & zone risk</Text>
          <Text style={[styles.subtitle, { color: colors.textSecondary }]}>
            Cadence {gaitAnalysis.cadenceSpm.toFixed(0)} spm · peak {gaitAnalysis.peakKpa.toFixed(0)} kPa · IWGDF {gaitAnalysis.iwgdfCategory} · zone {gaitAnalysis.highRiskZone}
          </Text>
          <View style={{ flexDirection: 'row', marginTop: 12, gap: 8 }}>
            <TouchableOpacity
              style={[styles.alertBadge, { backgroundColor: colors.primary, paddingHorizontal: 12 }]}
              onPress={() => router.push('/scan')}
            >
              <Text style={styles.alertBadgeText}>Wound scan</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[styles.alertBadge, { backgroundColor: colors.textSecondary, paddingHorizontal: 12 }]}
              onPress={() => router.push('/clinician')}
            >
              <Text style={styles.alertBadgeText}>Clinician report</Text>
            </TouchableOpacity>
          </View>
        </View>
